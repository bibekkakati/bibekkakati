---
title: "Secure Local Storage in Flutter"
publishedAt: "2021-05-12"
updatedAt: "2021-05-12"
author: "Bibek Kakati"
summary: "Learn how to implement secure local storage in flutter apps using the flutter_secure_storage package."
image: ""
---

In this article, we will see how to implement secure local storage in flutter apps.
...
We will use a flutter plugin called [flutter\_secure\_storage](https://pub.dev/packages/flutter_secure_storage) .
...
To insert a key-value pair in the storage, we will use the `write` method. We need to pass `key` and `value` to this method.
...
To read a `value` for a particular `key`, we will use the `read` method. We need to pass the `key` to this method.
...
To delete an entry, we will use the `delete` method. We need to pass the `key` to this method.
